    ...
    <div class="col-md">
      <div class="card my-4">
        <div class="card-body">
          ...
          <form action="{{ route('pinjaman-setoran.store') }}" method="post">
            @csrf
            <div class="form-group">
              <label for="tgl_setoran">Tanggal Setoran</label>
              <input type="date" name="tgl_setoran" id="tgl_setoran" class="form-control" placeholder="Tanggal Peminjaman Setoran" value="{{ old('tgl_setoran') }}">
            ...
            <div class="form-group">
              ...
              <select name="pinjaman_id" id="besar_pinjaman" class="form-control">
                ...
                <option value="{{ $p->id}}">{{ $p->besar_pinjaman }}</option>
                @endforeach
              </select>
            </div>
            <div class="form-group">
              <label for="setoran_pokok">Setoran Pokok</label>
              <input type="text" name="setoran_pokok" id="setoran_pokok" class="form-control" value="{{ old('setoran_pokok') }}" placeholder="Setoran Pokok">
              <small class="text-danger">{{ $errors->first('setoran_pokok') }}</small>
            </div>
            <div class="form-group">
              <label for="setoran_bunga">Setoran Bunga</label>
              <input type="text" name="setoran_bunga" id="setoran_bunga" class="form-control" value="{{ old('setoran_bunga') }}" placeholder="Setoran Bunga">
              <small class="text-danger">{{ $errors->first('setoran_bunga') }}</small>
            </div>
            <div class="form-group">
              <label for="denda">Denda</label>
              <input type="text" name="denda" id="denda" class="form-control" value="{{ old('denda') }}" placeholder="Denda">
